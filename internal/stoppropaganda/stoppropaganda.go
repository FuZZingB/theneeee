...
var (
	...
	flagDialsPerSecond  = fs.Int("dialspersecond", 9500, "maximum amount of TCP SYN packets sent per second (from fasthttp)")
	flagDNSTimeout      = fs.Duration("dnstimeout", time.Second, "timeout of DNS request")
	flagDNSWorkers      = fs.Int("dnsworkers", 1000, "DOS each DNS server with this amount of workers")
	...
	flagWorkers         = fs.Int("workers", 10000, "DOS each website with this amount of workers")
)
